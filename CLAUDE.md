...
│   │                           CommitGraph (virtualized, context menu incl. revert),
...
├── sidebar/Sidebar.tsx       ← SECTION STATE lives in ui.sidebarSections (persisted map
...
│                               carries the label, list/tree toggles, — in tree mode —
│                               "Expand all folders"/"Collapse all folders" (they bump
│                               ui.fileTreeFold {epoch, mode}; every mounted
│                               components/FileTree resets its collapsed set from it, so
│                               one pair of buttons serves the working copy AND commit
│                               details) and — in commit mode — the "Back to working
│                               copy" X, so CommitDetails has NO close button of its
│                               own), CommitDetails (owner
